    ...
    return (
        <>
            <ScrollView style={{ marginTop: 50 }}>
                ...
                <Text style={styles.informationTextStyle}>{item.resturantName} ({truncate(item.resturantAddress, 25)})</Text>
                <View style={styles.headerContainer}>
                    <Text style={styles.quantityTextStyle}>{quantity} items</Text>
                    <View style={styles.textContainer}>
                        <Text style={[styles.textStyle, { color: "grey" }]}>Subtotal: </Text>
                        <Text style={styles.textStyle}>{"\u00A3"}{getTotal()}</Text>
                    </View>
                </View>
                ...
                <View style={styles.titleTextContainer}>
                    ...
                    <Image source={{ uri: item.uri || item.image }} style={[styles.imageStyle, item.uri && { resizeMode: "contain" }]} />
                </View>
                <View style={styles.totalTextContainer}>
                    ...
                    <Text style={styles.totalTextStyle}>{"\u00A3"}{getTotal()}</Text>
                </View>
                <View style={[styles.borderStyle, { marginTop: 20 }]} />
                ...
                <View style={styles.subTotalTextContainer}>
                    ...
                    <Text style={styles.getTotalFunctionTextStyle}>{"\u00A3"}{getTotal()}</Text>
                </View>
            </ScrollView>
            <View style={styles.buttonBorderStyle} />
            <Pressable style={[styles.buttonContainer, { backgroundColor: "black" }]} onPress={() => navigation.navigate("Checkout", {
                item: item,
                getTotal: getTotal(),
                dessertPrice: dessertPrice
            })}>
                <Text style={[styles.buttonTextStyle, { color: "white" }]}>Go to checkout</Text>
            </Pressable>
            <Pressable style={[styles.buttonContainer, { backgroundColor: "#eaeaea" }]} onPress={() => navigation.navigate("Cart", {
                ...
                <Text style={[styles.buttonTextStyle, { color: "black" }]}>Add items</Text>
            </Pressable>
        </>
    );
}
