            ...
            <View>
                {items.map((item, index) => (
                    <Pressable key={index} onPress={() => navigation.navigate("OrderInfo", {
                        ...
                        <View style={[styles.basketItemsContainer, items.indexOf(item) === items.length - 1 && { marginBottom: 14 }]}>
                            <Image source={{ uri: item.resturantImage }} style={styles.imageStyle} />
                            <View style={styles.informationContainer}>
                                <View style={{ marginLeft: 12 }}>
                                    <View style={styles.informationSubContainer}>
                                        <Text style={styles.nameTextStyle}>{truncate(item.resturantName, 18)}</Text>
                                        <Text style={styles.addressTextStyle}>({truncate(item.resturantAddress, 12)})</Text>
                                    </View>
                                    <View style={{ marginTop: 8 }}>
                                        <Text style={styles.textStyle}>{item.quantity} items • {"\u00A3"}{(item.total).toFixed(2)}</Text>
                                        <Text style={styles.textStyle}>Deliver to San Antonio</Text>
                                    </View>
                                </View>
                            </View>
